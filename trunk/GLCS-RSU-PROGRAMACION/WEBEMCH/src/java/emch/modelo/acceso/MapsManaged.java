...
public class MapsManaged {
    ...
    public List listarRutaAsignadaTodos(TDespachodet selecteddespachodet) {
        ...
        qry = sesion.createQuery("SELECT ab FROM TAsignarruta  ab where ab.TDespachodet.id.cdDespacho='"+selecteddespachodet.getId().getCdDespacho()+"' and ab.TDespachodet.id.cdRuc='"+selecteddespachodet.getId().getCdRuc()+"'" );
    ...
    private String obtenerIDAsignarruta() {
     ...
     public List listarRutaAsignadaxCamion(TDespachodet selecteddespachodet) {
        List<TAsignarruta> listaLiq = null;
        /*  try {*/
        sesion = HibernateUtil.getSessionFactory().openSession();
